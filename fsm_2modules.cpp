...
void fsm(int line , vector<tuple<string, string, string>>& fsmstates,string null,string default_state)
{
    int l = line;
    ...
    while (getline(file, str))
    {
        ...
        for (int i = 0; i < str.length(); i++)
        {
            ...
            else if (word == "case")
            {
                ...
                fsm(cnt,fsmstates,null,default_state);

                if(fsmstates.size()!=0){
                    string file_name = "fsm"+to_string(file_count)+".txt";
                    file_count++;
                    ofstream myfile;
